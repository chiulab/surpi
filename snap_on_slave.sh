...
END1=$(date +%s)
diff1=$(( END1 - START1 ))
echo "done compressing file to transfer to slaves in $diff1 seconds"
...
echo "done transferring data to slaves in $diff2 seconds"

#run SNAP on slaves
START3=$(date +%s)
echo "running SNAP on slaves..."
COUNTER=0
while read line; do
...
END3=$(date +%s)
diff3=$(( END3 - START3 ))
echo "done running SNAP on slaves and transferring data from slaves to master in $diff3 seconds"
...
END5=$(date +%s)
diff5=$(( END5 - START5 ))
echo "done merging SAM files on master in $diff5 seconds"
